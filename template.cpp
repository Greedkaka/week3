...
//     console::draw(x1,0,WALL_HORIZONTAL_STRING);
//   console::draw(BOARD_SIZE-1,0,WALL_RIGHT_TOP_STRING);
  
//   for(int y1=1; y1<BOARD_SIZE-1; y1++){
//     console::draw(0,y1,WALL_VERTICAL_STRING);
//     console::draw(BOARD_SIZE-1,y1,WALL_VERTICAL_STRING);
//   }

//   console::draw(0,BOARD_SIZE-1,WALL_LEFT_BOTTOM_STRING);
//   for (int x1=1; x1<BOARD_SIZE-1; x1++)
//     console::draw(x1,BOARD_SIZE-1,WALL_HORIZONTAL_STRING);
...
void game() {
 ...
 // map();
  ...
  for (int x1=1; x1<BOARD_SIZE-1; x1++)
    console::draw(x1,0,"━");
  console::draw(BOARD_SIZE-1,0,"┓");
  
  for(int y1=1; y1<BOARD_SIZE-1; y1++){
    console::draw(0,y1,"┃");
    console::draw(BOARD_SIZE-1,y1,"┃");
  }

  console::draw(0,BOARD_SIZE-1,"┗");
  for (int x1=1; x1<BOARD_SIZE-1; x1++)
    console::draw(x1,BOARD_SIZE-1,"━");
  console::draw(BOARD_SIZE-1,BOARD_SIZE-1,"┛");
  int a;
  std::cin>>a;
  while (true) {
    // 화면을 지운다.
